...
class StoreController extends Controller
{
    ...
    function save(Request $request)
    {
        ...
        if ($request->store_id != null && !empty($request->store_id)) {
            ...
            if ($this->Stores->updateIt($stores) === true) {
                ...
                $result = ['status' => 1, 'action' => 'update', 'message' => 'Données non sauvegarder', 'error' => $this->Stores->updateIt($stores)];
            }
        } else {
            $stores = new Dao_store(
                null,
            ...
            if ($this->Stores->createIt($stores) === true) {
                ...
                $result = ['status' => 1, 'action' => 'create', 'message' => 'Données non sauvegarder', 'error' => $this->Stores->createIt($stores)];
            }
        }
    ...
    function delete($id)
    {
        ...
        if ($this->Stores->dropIt($id) === true) {
            ...
            $results = ['status' => 1, 'message' => 'Données non supprimée', 'error' => $this->Stores->dropIt($id)];
        }
